...
if (!class_exists('WP_Bootstrap_Navwalker')) :
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu
	{
		...
		public function start_lvl(&$output, $depth = 0, $args = null)
		{
			...
			$classes[] = 'dropright'; // class to add child menu to right side

			//$classes[] = "dropleft"; // class to add child menu to left side
		...
		public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
		{
			...
			if ($this->has_children) {
				if($depth === 0) {
					$icon_classes[] = '';
				}// "ti-angle-down ml-1"; //add arrow to first level only
				else {
					$linkmod_classes[] = 'dropdown-toggle';
				} //add > arrow to submenu level only
			}
